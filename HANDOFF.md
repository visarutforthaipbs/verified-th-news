...
Last updated: 2026-07-11 (evening). State of the project for anyone (human
or agent) picking this up in a new session.
...
- **popmacmini** (`ssh popmacmini`, Intel, 24/7) — **production home.**
  ...
  `com.thverify.server` (uvicorn 0.0.0.0:8942, KeepAlive) and
  `com.thverify.daily-sync` (03:30: delta sync → exports → index).
...
2. When enough labels: fine-tune WangchanBERTa baseline on the exports,
...
3. Gold eval set: human-verify a stratified ~500–800 sample across all
   sources/years (frozen benchmark).
4. Possible products: LINE bot wrapping /check; public data-story report
   (data/exports/REPORT.md has the insight tables: scam topics, AI-content
   growth 20→263 mentions/yr 2022→2025, category shifts).
